...
check_system() {
    if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
        echo -e "${OK} ${GreenBG} The current operating system is Centos ${VERSION_ID} ${VERSION} ${Font}"
        INS="yum"
    elif [[ "${ID}" == "debian" && ${VERSION_ID} -ge 8 ]]; then
        echo -e "${OK} ${GreenBG} The current operating system is Debian ${VERSION_ID} ${VERSION} ${Font}"
        INS="apt"
        $INS update
        ## 添加 Nginx apt源
    elif [[ "${ID}" == "ubuntu" && $(echo "${VERSION_ID}" | cut -d '.' -f1) -ge 16 ]]; then
        echo -e "${OK} ${GreenBG} The current operating system is Ubuntu ${VERSION_ID} ${UBUNTU_CODENAME} ${Font}"
        INS="apt"
        ...
        echo -e "${Error} ${RedBG} The current operating system is ${ID} ${VERSION_ID} Not in the list of supported systems, installation cancelled. ${Font}"
        exit 1
    fi
...
is_root() {
    if [ 0 == $UID ]; then
        echo -e "${OK} ${GreenBG} The current user is the root user, entering the installation process ... ${Font}"
        sleep 3
    else
...
domain_check() {
    ...
    if [[ ${wgcfv4_status} =~ "on"|"plus" ]] || [[ ${wgcfv6_status} =~ "on"|"plus" ]]; then
        ...
        echo -e "${OK} ${GreenBG} wgcf-warp closed  ${Font}"
    fi
...
uninstall_all() {
    ...
    if [[ -d $nginx_dir ]]; then
        echo -e "${OK} ${Green} Are you sure to uninstall Nginx [Y/N]? ${Font}"
...
delete_tls_key_and_crt() {
    ...
    echo -e "${OK} ${GreenBG} Empty certificate legacy files ${Font}"
}
judge_mode() {
...
menu() {
    update_sh
    echo -e "\t V2ray Installation Administration Script ${Red}[${shell_version}]${Font}"
    echo -e "\t---authored by wulabing Translated by ZWSQ---"
    echo -e "\thttps://github.com/zwsq\n"
    echo -e "Currently installed version:${shell_mode}\n"

    echo -e "—————————————— Installation ——————————————"""
    echo -e "${Green}0.${Font}  Upgrade the script"
    echo -e "${Green}1.${Font}  Install V2Ray (Nginx+ws+tls+vmess)"
    echo -e "${Green}2.${Font}  Install V2Ray (http/2)"
    echo -e "${Green}3.${Font}  Upgrade V2Ray core"
    echo -e "—————————————— Change Configurations ——————————————"
    ...
    echo -e "${Green}11.${Font} Install the 4-in-1 bbr sharp installation script"
    ...
    17)
        ...
        ;;
    18)
        read -rp "Please enter the camouflage path (note! no slashes eg:ray):" camouflage_path
        modify_camouflage_path
        start_process_systemd
        ;;
    *)
